import { ArrowUpRightIcon, BikeIcon, ChevronDownIcon, LogOutIcon, MapPinIcon, MenuIcon, PackageIcon, SearchIcon, ShieldCheckIcon, ShoppingCartIcon, UserIcon, XIcon } from "lucide-react";
...
const Navbar = () => {
    ...
    const user: any ={name: "Kasun Weerasekara", email: "kasun.weerasekara@example.com",
        isAdmin: true
    }
  ...
  return (
    <nav className="bg-white sticky top-0 z-50 border-b border-app-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center j
      ustify-between h-16 gap-4">
        {/* Logo */}
        <Link to="/" className="flex items-centergap-2 text-[22px] font-medium shrink-0">
          <BikeIcon size={24}/> Weerasekara Super Store
        </Link>
        ...
        <div className="w-full flex items-center justify-end gap-4 lg:gap-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="relative p-2 rounded-xl" onClick={()=>setIsCartOpen(true)}>
              ...
              -right-1 size-5 bg-app-orange text-white 
            ...
            <div className="relative">
              {user ? (
                ...
                <div className="flex-center gap-2">
                  <Link to="/login" className="hidden md:flex items-white gap-2 px-4 py-2
                  text-sm font-medium text-white bg-green-950 rounded-full 
              ...
              {userMenuOpen && (
                ...
                <div className="fixed inset-0 z-40" onClick={()=>
                  setUserMenuOpen(false)}/>
                  <div className="absolute right-0 mt-2.5 w-56 bg-white 
                  rounded-xl shadow-lg border border-app-border py-2 z-50 animate-fade-in">
                    {user && (
                    ...
                    <div onClick={()=>setUserMenuOpen(false)}>
                      ...
                      <Link to="/products" className="dropdown-link md:hidden" >
                        <ArrowUpRightIcon size={16}/> My Products
                      </Link>
                     
                      <Link to="/deals" className="dropdown-link md:hidden" >
                        <ArrowUpRightIcon size={16}/> My Deals
                      </Link>
                      
                    {user?.isAdmin && (
                      <Link to="/admin/products" className="dropdown-link" >
                        <ShieldCheckIcon className="text-app-orange-dark" size={16}/> 
                        <span className="text-app-orange-dark">Admin Dashboard</span>
                      </Link>
                    )}
                    {user && (
                      <div className="border-t border-app-border mt-2 pt-2">
                        ...
                        text-app-error hover:bg-red-50 transition-colors w-full" >
