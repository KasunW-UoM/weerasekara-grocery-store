...
const OrderTracking = () => {
  ...
  useEffect(() => {
    setOrder(dummyDashboardOrdersData.find((o) => o._id === id) as
      any)
    setLoading(false)
  }, [id, navigate])
  ...
  return (
    <div className="min-h-screen mb-20 bg-app-cream">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        text-sm text-app-text-light hover:text-app-green mb-6 transition-colors">
            ...
            {order?.deliveryPartner && order.status !== "Delivered" &&
              order.status !== "Cancelled" && (
                <div className="bg-white rounded-2xl p-5 flex items-center justify-between">
                  ...
                  <a href={`tel:${order.deliveryPartner.phone}`} className="p-2.5 bg-app-cream
                rounded-xl hover:bg-app-cream-dark transition-colors">
            ...
            <div className="bg-white rounded-2xl p-5">
                ...
                {order?.items.map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-app-green trancate">{item.name}</p>
              ...
              <div className="mt-4 pt-3 border-t border-app-border space-y-1.5 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span>{order?.deliveryFee === 0 ? "Free" : `${currency}${order?.deliveryFee.toFixed(2)}`}
                  </span>
                </div>

                <div className="flex justify-between">
                  <span className="text-app-text-light">Tax</span>
                  <span>{currency}{order?.tax.toFixed(2)}
                  </span>
                </div>
                ...
                font-semibold text-app-green">
                  <span>Total</span>
                  <span>{currency}{order?.total.toFixed(2)}
                  </span>
                </div>
              </div>
            </div>
