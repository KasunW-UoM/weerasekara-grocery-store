...
const getAdminStatus = (email) => {
    if (!email)
        return false;
    const adminEmail = process.env.ADMIN_EMAILS ? process.env.ADMIN_EMAILS.split(",").map((e) => e.trim().toLowerCase()) : [];
...
export const login = async (req, res) => {
    ...
    if (!email || !password) {
        return res.status(400).json({ message: "Please provide email and password fields" });
    }
